...
--- The parsers in this file return data in a format any frontend can understand.
  ...
  if file then
    ...
    for line in file:lines() do
      ...
      if target then
        ...
        table.insert(options, { text = 'Make ' .. target, value = target, bau = 'make' })
...
local function get_cmake_opts(path)
  ...
  if file then
    ...
    for target in content:gmatch 'add_executable%s*%(%s*([%w_-]+)' do
      table.insert(options, { text = 'CMake ' .. target, value = target, bau = 'cmake' })
    end
...
local function get_meson_opts(path)
  ...
  if file then
    ...
    for target in content:gmatch 'executable%s*%(.-[\'"]([%w_]+)[\'"]' do
      table.insert(options, { text = 'Meson ' .. target, value = target, bau = 'meson' })
    end
  ...
  if file then
    ...
    for line in file:lines() do
      ...
      if task_match then
        ...
        table.insert(options, { text = 'Gradle ' .. task_name, value = task_name, bau = 'gradle' })
      elseif in_task then
...
function M.get_bau_opts()
  ...
  -- make
  vim.list_extend(options, get_makefile_opts(working_dir .. utils.os_path '/Makefile'))

  -- cmake
  vim.list_extend(options, get_cmake_opts(working_dir .. utils.os_path '/CMakeLists.txt'))

  -- meson
  vim.list_extend(options, get_meson_opts(working_dir .. utils.os_path '/meson.build'))

  -- gradle
  vim.list_extend(options, get_gradle_cmd_opts(working_dir .. utils.os_path '/build.gradle.kts'))
  vim.list_extend(options, get_gradle_opts(working_dir .. utils.os_path '/build.gradle.kts'))

  -- nodejs
  vim.list_extend(options, get_nodejs_opts(working_dir .. utils.os_path '/package.json'))
